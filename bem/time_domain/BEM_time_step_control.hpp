...
bool is_geometry_step_failure_reason(const std::string& reason) {
  std::string lower;
  lower.reserve(reason.size());
  for (char c : reason)
    lower.push_back(static_cast<char>(std::tolower(static_cast<unsigned char>(c))));
  const std::array<std::string, 8> keys = {
      "bcinterface", "waterline", "tiny face", "subsurface",
      "remesh", "mesh", "contact", "geometry"};
...
struct StepRetryState {
  ...
  RetryAction handle_step_failure(const step_failure& e, const int step_retry,
    ...
    if (step_retry >= max_step_retries) {
      const bool geometry_failure = is_geometry_step_failure_reason(e.reason);
      std::cerr << Yellow << "[step_reject] writing failure snapshot before "
                << (geometry_failure ? "stopping retry for geometry/contact failure" : "degraded continuation")
                << colorReset << std::endl;
      write_failure_snapshot_fn(current_rk_step, step_retry);
      if (geometry_failure) {
        std::cerr << Yellow << "[step_reject] max retries exceeded for geometry/contact failure. "
                  << "Skipping degraded continuation for time_step " << time_step
                  << "." << colorReset << std::endl;
        return RetryAction::BreakStep;
      }
