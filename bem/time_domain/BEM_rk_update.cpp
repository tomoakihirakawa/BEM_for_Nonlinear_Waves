...
void applyAbsorptionAndPush(const std::vector<BEM_DOF_Base*>& fluid_nodes,
  ...
  for (auto* node : fluid_nodes) {
    ...
    // phi push は BIE DOF を持つ場合のみ（linear 要素の line では phi_mid は
    // BIE 未知数でないため、端点平均から後で導出する）。
    if (hasAnyDirichletBoundaryState(node) && node->hasActiveBieDof()) {
...
void applyInterpolationRelocation(const std::vector<Network*>& FluidObject,
  ...
  for (auto water : FluidObject) {
    ...
    auto interpolateAndMove = [&](auto* entity) {
      // phi interpolation は BIE DOF を持つ entity のみ（linear 要素の line の
      // phi_mid は BIE 未知数でないので、後段で端点平均から導出する）。
      if (entity->hasActiveBieDof() && hasAnyDirichletBoundaryState(entity) && entity->relocation_face) {
    ...
    // 5. Apply to all line midpoints (要素タイプ非依存)
    for (auto* l : water->getBoundaryLines())
