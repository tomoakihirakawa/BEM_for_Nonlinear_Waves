...
void applyAbsorptionAndPush(const std::vector<BEM_DOF_Base*>& fluid_nodes,
  ...
  for (auto* node : fluid_nodes) {
    ...
    if (node->absorbedBy != nullptr) {
      ...
      if (!hasAnyDirichletBoundaryState(node) && node->BCInterface) {
        throw std::runtime_error("Error: Absorption for BCInterface nodes without Dirichlet faces is not supported. Please ensure that all BCInterface nodes have at least one Dirichlet face or remove the BCInterface classification.");
      }
    }
...
void applyInterpolationRelocation(const std::vector<Network*>& FluidObject,
  ...
  for (auto water : FluidObject) {
    ...
    auto interpolate_scalar = [&](networkFace* f, const double t0, const double t1, bool phi0_phin1 /*0 or 1*/) -> double {
      ...
      } else if (surface == NodeRelocationSurface::pseudo_quadratic) {
