...
// node_relocation_surface is a global defined in main_time_domain.cpp (before this header).
// It is temporarily modified during IC mesh smoothing, then restored.
  ...
  for (auto& water : FluidObject) {
    ...
    // Step 3: Set φ from theory at the final (post-smoothing) node positions
    {
      ...
      std::cout << Green << "[initial_condition] node classification: "
                << "Dirichlet=" << n_dirichlet << " BCInterface=" << n_corner
