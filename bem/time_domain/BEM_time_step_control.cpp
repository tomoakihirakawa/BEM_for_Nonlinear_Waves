#include "BEM_time_step_control.hpp"
...
void StepRetryState::remember_rejected_face(const step_failure& e) {
...
void StepRetryState::collapse_repeatedly_rejected_faces(Network& water, int step_retry) {
  if (step_retry <= 0)
    return;
...
RetryAction StepRetryState::handle_step_failure(const step_failure& e,
                                                int step_retry,
                                                double dt,
