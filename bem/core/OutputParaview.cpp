...
void mk_vtu_quadratic(const std::string &filename, const V_netFp &Faces, const VV_VarForOutput &VV_name_comp_mapPVd) {
  try {
    ...
    // Unified debug output: render every face as a 6-node quadratic subdivision
    // using l->X_mid for edge midpoints regardless of element type.
    // This exposes the raw X_mid state — if linear leaves X_mid uninitialized,
    // midpoints show up at the origin, making the bug visible.
    for (const auto &f : Faces) {
      ...
      const Tddd mid0 = l0->X_mid;
      const Tddd mid1 = l1->X_mid;
      const Tddd mid2 = l2->X_mid;

      auto [pa0, pb0] = l0->getPoints();
      auto [pa1, pb1] = l1->getPoints();
      auto [pa2, pb2] = l2->getPoints();
      all_nodes.push_back({l0, mid0, nullptr, pa0, pb0, l0, false});
      all_nodes.push_back({l1, mid1, nullptr, pa1, pb1, l1, false});
      all_nodes.push_back({l2, mid2, nullptr, pa2, pb2, l2, false});
