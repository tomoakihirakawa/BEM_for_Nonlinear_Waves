...
inline bool shell_precheck_normals(Network *net, std::string &warning) {
  for (auto *line : net->getBoundaryLines()) {
    ...
    if (dot < -0.1) {
      warning = "skip shell visualization for " + net->getName() + ": reversed boundary-face normals detected on non-BCInterface edge";
      return false;
    }
  }
  return true;
...
inline bool aft_new_face_intersects_boundary(Network *net,
  ...
  auto gather = [&](const Tddd &x) {
    net->applyFacesNear(x, query_range, [&](networkFace *face) {
      if (face)
...
inline AFTCandidateVisualizationData build_aft_candidate_visualization(Network *seed_net) {
  ...
  seed_net->makeFaceBVH(bucket_spacing);
...
inline double output_env_double(const char* name, const double fallback) {
  const char* raw = std::getenv(name);
  if (!raw)
    return fallback;
  char* end = nullptr;
  const double value = std::strtod(raw, &end);
  return (end != raw && std::isfinite(value)) ? value : fallback;
}

inline double output_face_edge_scale(const networkFace* f) {
  if (!f)
    return 0.0;
  double max_len = 0.0;
  ...
  return std::max({Norm(p0->getPosition() - p1->getPosition()),
                   Norm(p1->getPosition() - p2->getPosition()),
                   Norm(p2->getPosition() - p0->getPosition())});
}

struct FluidPairContactOutput {
  std::unordered_map<Network*, uomap_F_d> face_marker;
  std::unordered_map<Network*, uomap_F_d> face_distance;
  std::unordered_map<Network*, uomap_F_d> node_face_marker;
  std::unordered_map<Network*, uomap_F_d> node_face_distance;
  std::unordered_map<Network*, uomap_F_d> node_face_abs_normal_dot;
  std::unordered_map<Network*, uomap_F_d> node_face_bary_min;
  std::unordered_map<Network*, uomap_F_d> node_face_adjacent;
  std::unordered_map<Network*, uomap_F_d> node_face_nonadjacent;
  std::unordered_map<Network*, uomap_F_d> node_face_type;
  std::size_t broad_pairs = 0;
  std::size_t contact_pairs = 0;
  std::size_t node_face_candidates = 0;
  std::size_t node_face_contacts = 0;
};

inline bool output_face_shares_point(const networkFace* a, const networkFace* b) {
  if (!a || !b)
    return false;
  const auto [a0, a1, a2] = a->getPoints();
  const auto [b0, b1, b2] = b->getPoints();
  return a0 == b0 || a0 == b1 || a0 == b2 ||
         a1 == b0 || a1 == b1 || a1 == b2 ||
         a2 == b0 || a2 == b1 || a2 == b2;
}

inline double output_abs_normal_dot(const networkFace* a, const networkFace* b) {
  if (!a || !b)
    return 0.0;
  const double na = Norm(a->normal);
  const double nb = Norm(b->normal);
  if (!(na > 0.0) || !(nb > 0.0) || !std::isfinite(na) || !std::isfinite(nb))
    return 0.0;
  return std::clamp(std::abs(Dot(a->normal, b->normal) / (na * nb)), 0.0, 1.0);
}

inline void output_update_face_scalar_min(uomap_F_d& map, networkFace* f, const double value) {
  auto& prev = map[f];
  if (!(prev >= 0.0) || value < prev)
    prev = value;
}

inline FluidPairContactOutput build_fluid_pair_contact_output(const std::vector<Network*>& fluids) {
  FluidPairContactOutput out;
  std::vector<Network*> nets;
  nets.reserve(fluids.size());
  for (auto* net : fluids) {
    if (!net)
      continue;
    nets.push_back(net);
    auto& marker = out.face_marker[net];
    auto& distance = out.face_distance[net];
    auto& nf_marker = out.node_face_marker[net];
    auto& nf_distance = out.node_face_distance[net];
    auto& nf_abs_normal_dot = out.node_face_abs_normal_dot[net];
    auto& nf_bary_min = out.node_face_bary_min[net];
    auto& nf_adjacent = out.node_face_adjacent[net];
    auto& nf_nonadjacent = out.node_face_nonadjacent[net];
    auto& nf_type = out.node_face_type[net];
    for (auto* f : net->getBoundaryFaces()) {
      marker[f] = 0.0;
      distance[f] = -1.0;
      nf_marker[f] = 0.0;
      nf_distance[f] = -1.0;
      nf_abs_normal_dot[f] = -1.0;
      nf_bary_min[f] = -1.0;
      nf_adjacent[f] = 0.0;
      nf_nonadjacent[f] = 0.0;
      nf_type[f] = 0.0;
    }
  }
  if (nets.empty())
  ...
  const double range_factor = std::max(0.0, output_env_double("BEM_FLUID_PAIR_CONTACT_RANGE_FACTOR", 0.4));
  const double absolute_range = output_env_double("BEM_FLUID_PAIR_CONTACT_RANGE", -1.0);
  const double node_face_range_factor =
      std::max(0.0, output_env_double("BEM_FLUID_NODE_FACE_CONTACT_RANGE_FACTOR", range_factor));
  const double node_face_absolute_range = output_env_double("BEM_FLUID_NODE_FACE_CONTACT_RANGE", absolute_range);
  const double adjacent_bary_min =
      std::clamp(output_env_double("BEM_FLUID_NODE_FACE_ADJACENT_BARY_MIN", 0.02), 0.0, 1.0 / 3.0);
  for (auto* net : nets)
    net->makeBoundingVolumeHierarchyForBoundaryFaces();

  std::unordered_map<networkFace*, double> face_scale;
  auto get_scale = [&](networkFace* f) {
    auto it = face_scale.find(f);
    if (it != face_scale.end())
      return it->second;
    const double scale = output_face_edge_scale(f);
    face_scale[f] = scale;
    return scale;
  };

  for (auto* source : nets) {
    for (auto* own_face : source->getBoundaryFaces()) {
      if (!own_face)
        continue;
      const auto [p0, p1, p2] = own_face->getPoints();
      const std::array<networkPoint*, 3> points{p0, p1, p2};
      for (auto* p : points) {
        if (!p)
          continue;
        const double point_range = (node_face_absolute_range > 0.0)
                                       ? node_face_absolute_range
                                       : node_face_range_factor * std::max(localEdgeLength(p), get_scale(own_face));
        if (!(point_range > 0.0) || !std::isfinite(point_range))
          continue;

        for (auto* target_net : nets) {
          if (!target_net)
            continue;
          target_net->applyBoundaryFacesNear(p->getPosition(), point_range, [&](networkFace* target_face) {
            if (!target_face || target_face == own_face)
              return;
            ++out.node_face_candidates;

            const auto [w0, w1, nearest, nearest_normal] = Nearest_(p->getPosition(), ToX(target_face));
            const double w2 = 1.0 - w0 - w1;
            const double bary_min = std::min({w0, w1, w2});
            const double distance = Norm(nearest - p->getPosition());
            if (!std::isfinite(distance) || distance > point_range)
              return;

            const bool same_network = target_face->getNetwork() == source;
            const bool adjacent = same_network && output_face_shares_point(own_face, target_face);
            const double angle = contactAcceptanceAngle(distance, point_range, 70.0, 50.0);
            const double abs_dot = output_abs_normal_dot(own_face, target_face);
            const bool normal_ok = abs_dot >= std::cos(angle);
            const bool tiny_distance = distance <= 0.05 * point_range;

            bool contact = false;
            double type = 0.0;
            if (adjacent) {
              contact = (bary_min >= adjacent_bary_min) && (normal_ok || tiny_distance);
              type = 2.0;
            } else {
              contact = normal_ok || tiny_distance;
              type = 1.0;
            }
            if (!contact)
              return;

            ++out.node_face_contacts;
            auto update = [&](Network* net, networkFace* f) {
              out.node_face_marker[net][f] = 1.0;
              output_update_face_scalar_min(out.node_face_distance[net], f, distance);
              out.node_face_abs_normal_dot[net][f] = std::max(out.node_face_abs_normal_dot[net][f], abs_dot);
              out.node_face_bary_min[net][f] = std::max(out.node_face_bary_min[net][f], bary_min);
              out.node_face_type[net][f] = std::max(out.node_face_type[net][f], type);
              if (adjacent)
                out.node_face_adjacent[net][f] = 1.0;
              else
                out.node_face_nonadjacent[net][f] = 1.0;
            };
            update(source, own_face);
            update(target_face->getNetwork(), target_face);
          });
        }
      }
    }
  ...
  for (std::size_t ia = 0; ia < nets.size(); ++ia) {
    for (std::size_t ib = ia + 1; ib < nets.size(); ++ib) {
      auto* a = nets[ia];
      auto* b = nets[ib];
      double max_pair_range = absolute_range;
      if (!(max_pair_range > 0.0)) {
        max_pair_range = 0.0;
        for (auto* f : a->getBoundaryFaces())
          max_pair_range = std::max(max_pair_range, range_factor * get_scale(f));
        for (auto* f : b->getBoundaryFaces())
          max_pair_range = std::max(max_pair_range, range_factor * get_scale(f));
      }
      if (!(max_pair_range > 0.0))
        continue;

      a->applyBoundaryFacePairsNear(*b, max_pair_range, [&](networkFace* fa, networkFace* fb) {
        ++out.broad_pairs;
        if (!fa || !fb)
          return;
        const double pair_range = (absolute_range > 0.0)
                                      ? absolute_range
                                      : range_factor * std::min(get_scale(fa), get_scale(fb));
        if (!(pair_range > 0.0))
          return;
        const auto [xa, xb] = ::Nearest(ToX(fa), ToX(fb));
        const double d = Norm(xa - xb);
        if (!(d <= pair_range))
          return;
        ++out.contact_pairs;
        auto update = [&](Network* net, networkFace* f) {
          out.face_marker[net][f] = 1.0;
          auto& prev = out.face_distance[net][f];
          if (!(prev >= 0.0) || d < prev)
            prev = d;
        };
        update(a, fa);
...
void write_step(const OutputContext &ctx, const std::map<std::string, outputInfo> &NetOutputInfo, const std::vector<Network *> &FluidObject, const std::vector<Network *> &RigidBodyObject, const std::vector<Network *> &SoftBodyObject, const std::unordered_set<networkFace *> & /*allFaces*/) {
  auto fluid_pair_contact = build_fluid_pair_contact_output(FluidObject);
  if (!FluidObject.empty()) {
    std::cout << "[fluid_pair_contact] step=" << ctx.time_step
              << " broad_pairs=" << fluid_pair_contact.broad_pairs
              << " contact_pairs=" << fluid_pair_contact.contact_pairs
              << " node_face_candidates=" << fluid_pair_contact.node_face_candidates
              << " node_face_contacts=" << fluid_pair_contact.node_face_contacts << std::endl;
  }
  ...
  for (auto *net : FluidObject) {
    ...
    auto data = dataForOutput(net, ctx.dt);
    if (auto mit = fluid_pair_contact.face_marker.find(net); mit != fluid_pair_contact.face_marker.end()) {
      data.push_back({"fluid_pair_contact_face", mit->second});
      data.push_back({"fluid_pair_contact_distance", fluid_pair_contact.face_distance[net]});
      data.push_back({"fluid_node_face_contact_face", fluid_pair_contact.node_face_marker[net]});
      data.push_back({"fluid_node_face_contact_distance", fluid_pair_contact.node_face_distance[net]});
      data.push_back({"fluid_node_face_contact_abs_normal_dot", fluid_pair_contact.node_face_abs_normal_dot[net]});
      data.push_back({"fluid_node_face_contact_bary_min", fluid_pair_contact.node_face_bary_min[net]});
      data.push_back({"fluid_node_face_contact_adjacent", fluid_pair_contact.node_face_adjacent[net]});
      data.push_back({"fluid_node_face_contact_nonadjacent", fluid_pair_contact.node_face_nonadjacent[net]});
      data.push_back({"fluid_node_face_contact_type", fluid_pair_contact.node_face_type[net]});
    }
    mk_vtu_quadratic((ctx.output_directory / filename).string(), net->getBoundaryFaces(), data);
    if (info.PVD) {
